...
function visibleLen(s) {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    if (c >= 0x2000 && c <= 0x2BFF) len += 1
    else if (c >= 0x2500 && c <= 0x27BF) len += 1
    else if (c >= 0x1F000 && c <= 0x1FFFF) len += 2
    else if (c >= 0xE000 && c <= 0xF8FF) len += 1
    else if (c < 32 || c === 127) { /* escape, skip */ }
    else len += 1
  }
  return len
}
function fill(c, n) { return c + '─'.repeat(Math.max(0, n)) + N }
function tag(l, c) { return c+'▌'+N+W+' '+l+' '+N }

function topBar() {
  const label = ' CAMBIO_LABS_V1.0 IFE 2026 AI LITERACY TRACK '
  return P+'╔══'+N+W+B+label+N+P+'══╗'+N
}
function topBarD() {
  const label = ' CAMBIO_LABS_V1.0 MODULE DETAIL '
  return P+'╔══'+N+W+B+label+N+P+'═══╗'+N
}
function botBar(s) {
  const sl = visibleLen(s)
  const rest = Math.max(0, _W - 6 - sl)
  return P+'╚═'+N+G+' '+s+N+P+' ═'+'═'.repeat(rest)+'╝'+N
}
function navBar(l, r) {
  const gap = Math.max(1, _W - 4 - visibleLen(l) - visibleLen(r))
  return P+'║'+N+G+' '+l+N+G+' '.repeat(gap)+r+N+P+' ║'+N
}
...
function listView() {
  ...
  for (let i = 0; i < MODULES.length; i++) {
    ...
    if (b.w !== lw) {
      ...
      lw = b.w
      const label = '┌─ '+WL[b.w]+' '
      const dash = _W - 6 - visibleLen(label)
      o += P+'║'+N+' '+T+label+fill(T, dash)+N+P+' ║'+N+'\n'
    }
    const sel = i === si
    ...
    let notes = ''
    if (b.ao && b.ao !== '—') notes += ' '+S+'[aot]'+N
    if (b.j && b.j.includes('empty shell')) notes += ' '+S+'[shell]'+N
    let line = hl+b.id.padEnd(4)+N+' '+hl+b.n+N+notes
    const maxW = _W - 11
    if (visibleLen(line) > maxW) {
      const maxName = maxW - 10
      line = hl+b.id+N+' '+hl+b.n.substring(0, Math.max(3, maxName))+'…'+N+notes
    }
    o += P+'║'+N+' '+bg+'│ '+N+mark+' '+line+rbg+' \n'
    o += P+'║'+N+' '+bg+'│ '+N+G+'  '+b.d+' · '+b.dt+' · '+b.du+N+rbg+' \n'
  }
  o += P+'║'+N+' '+fill(G, _W-4)+N+P+' ║'+N+'\n\n'

  // Preview pane
  const s = MODULES[si]
  const plabel = '── '+s.id+' · '+s.n+' '
  const pdash = _W - 6 - visibleLen(plabel)
  o += P+'║'+N+' '+P+plabel+fill(P, pdash)+N+P+' ║'+N+'\n'
  o += P+'║'+N+' '+G+'▸ Frame:'+N+'   '+s.f+'\n'
  o += P+'║'+N+' '+S+'▸ Take:'+N+'  '+S+s.t+N+'\n'
  o += P+'║'+N+' '+T+'▸ Build:'+N+'  '+T+s.a+N+'\n'
  const ji = G+'▸ Journey:'+N+' '+s.j
  const ai = G+'AOT:'+N+' '+s.ao
  o += P+'║'+N+' '+ji+G+'  │  '+ai+'\n'

  o += '\n'+botBar('↑↓ nav · ↵ select · /q quit')
  o += SH
  return o
}
...
function detailView(idx) {
  ...
  // Header
  const hline = '╔═ '+b.id+' '+b.n+' │ '+b.d+' · '+b.dt+' · '+b.du+' ═╗'
  o += '  '+P+hline+N+'\n'
  ...
  function sec(title, color, body) {
    const label = '┌─ '+title+' '
    const dash = _W - 6 - visibleLen(label)
    o += '  '+color+label+fill(color, dash)+N+'\n'
    o += '  '+color+'│'+N+'  '+body+'\n'
    o += '  '+color+fill(color, _W-6)+N+'\n\n'
  }

  sec('Conceptual Frame', G, b.f)
  sec('Critical Takeaway', S, b.t)
  sec('Technical Artifact', T, b.a)

  // ASCII art
  const alabel = '┌─ ASCII Art '
  const adash = _W - 6 - visibleLen(alabel)
  o += '  '+G+alabel+fill(G, adash)+N+'\n'
  for (const l of b.as.split('\n')) {
    o += '  '+G+'│'+N+l+'\n'
  }
  o += '  '+G+fill(G, _W-6)+N+'\n\n'

  // Commands
  const clabel = '┌─ Claude Code Session '
  const cdash = _W - 6 - visibleLen(clabel)
  o += '  '+P+clabel+fill(P, cdash)+N+'\n'
  for (const c of b.cm) {
    o += '  '+P+'│'+N+' '+P+'$ '+c.c+N+'\n'
    for (const l of c.o) {
      o += '  '+P+'│'+N+'  '+l+'\n'
